...
def get_ladder_translational_green(green,center_idx,path_to_ladder, large_N):
    ...
    # for idx in green_dict:
    #     if idx[0]+idx[1] <= 5:
    #         print(idx, green_dict[idx])
    # print(green_dict.keys(),len(green_dict.keys()))
    # print(original_green_dict[1,0])
    # print(original_green_dict[-1,0])
    # print(index_dict)
    # raise SystemExit(0)
    try:
...
N = 100
def read_overlap(case, U, N):
    overlap = np.zeros((N,N),dtype=np.complex_)
    for i in range(N):
        line = [0 for _ in range(N)]
        with open('.//overlap/overlap{}.txt'.format(case, U, i+1)) as f:
            j = 0
            for e in f:
                nums = e.strip().split()
...
def get_full_overlap(case, U, N):
    # overlap = np.zeros([N,N], dtype=np.complex)
    overlap = []
    for i in range(N):
        onsite = []
        with open('./overlap/{}_U{}/overlap_s{}.txt'.format(case, U, i+1)) as f:
            for line in f:
                content = line.strip().split()
                onsite.append(float(content[0])+1j*float(content[1]))
        
        overlap.append(onsite)
    overlap = np.array(overlap)   
    # print(overlap.shape)
    # plt.imshow(np.real(overlap))
    # plt.show()
    # raise SystemExit(0)
    return overlap
...
# for files in ['../ladder_system_test/{}_results'.format(args.doping)]:
    ...
    # for U in [1,2,4,8]:
    for U in [1]:
        # for case in ['U{}'.format(args.U)]:
        case = 'U{}_dag'.format(U)
        ...
        for m_idx, muti_num in enumerate(muti_num_grid):
            for i_idx, interval_step in enumerate(interval_step_grid):
                ...
                for i in range(tlimit_compare+1):
                    ...
                    # plt.imshow(np.real(Green_tn[i]))
                    # print(Green_snap[int(N/2)-2])
                    # print(Green_snap[int(N/2)-1])
                    # print(Green_snap[int(N/2)])
                    # print(Green_snap[int(N/2)+1])
                    # plt.plot(Green_tn[i][int(N/2),],label='shift')
                    
                    if t == 0:
                        overlap = get_ladder_translational_green(Green_snap, int(N/2)-1, path_to_ladder, large_N)
                        # plt.plot(overlap[int(N/2)-1,:])
                        # overlap = get_2d_translational_green(Green_snap, large_N)
                        # plt.plot(overlap[int(N/2)-1,:])
                    # plt.show()
                    # raise SystemExit(0)

                    # Green_tn[i] = get_2d_translational_green(Green_snap, large_N)
                    Green_tn[i] = get_ladder_translational_green(Green_snap, int(N/2)-1, path_to_ladder, Nreal*factor)
                    # plt.imshow(np.real(Green_tn[i]))
                    # plt.plot(Green_tn[i][int(N/2),],label='trans')
                    # plt.show()
                    # raise SystemExit(0)
                ...
                # plt.plot(overlap_pinv[int(large_N/2)-1,])

                full_overlap = get_full_overlap(doping, U, Nreal)
                full_overlap_pinv = np.linalg.pinv(full_overlap,rcond=1e-1)
                # plt.imshow(np.real(overlap_pinv))
                # plt.plot(overlap_pinv[int(N/2)-1,])
                # plt.show()
                # raise SystemExit(0)

                overlap_pinv = get_ladder_translational_green(full_overlap_pinv[int(N/2)-1], int(N/2)-1, path_to_ladder, Nreal*factor)
                # overlap_pinv = get_ladder_translational_green(overlap_pinv[int(large_N/2)-1], int(large_N/2)-1, path_to_ladder, Nreal*factor)
                # plt.imshow(np.real(overlap_pinv))
                # plt.plot(overlap_pinv[int(large_N/2)-1,])
                # plt.show()
                # raise SystemExit(0)
                ...
                # import os
                # green_save_dir = '{}/{}/Green_multi_re_t{}_factor{}'.format(new_dir_name, case, start_time, factor)
                ...
                #             f.write(str(np.real(re_Green_tn[t,int(large_GN/2)-1,i])) + ' ' + str(np.imag(re_Green_tn[t,int(large_GN/2)-1,i])) + '\n' )
                ...
                plt.figure()
                center_evolve_array = np.array([re_Green_tn[i,int(Nreal*factor/2)-1,:]*1.0 for i in range(tlimit)])
                # plt.imshow(np.flip(np.abs(center_evolve_array)), aspect='auto', extent=[0,large_N,0,25],cmap='jet')
                plt.imshow(np.flip(np.abs(ky0_green_array)), aspect='auto', extent=[0,len(ky0_green_array[0]),0,25],cmap='jet')
                # center_evolve[i] for i in r
                plt.tick_params('t(unit=0.1)')
                # plt.yticks(np.arange(0, 25, 1))
                plt.ylabel('t(unit=0.1)')
                plt.xlabel('x')
                # plt.savefig('./Edge_effect/ladder_trans_2N/center_evolve_array_{}_{}_ky0_green_mul{}.pdf'.format(doping, case, muti_num))
                plt.show()

                plt.figure()
                for i in range(0,23,4):
                    # plt.plot(np.real(re_Green_tn[i*10,int(Nreal*factor/2)-1,])+0.1*i)
                    plt.plot(np.real(ky0_green_array[i*10])+0.1*i)
                    plt.text(1,0.1*i,'t='+str(i))
                    # print(np.real(re_Green_tn[i*10,int(N/2)-1,]))
                plt.xlabel('x')
                # plt.savefig('./Edge_effect/ladder_trans_2N/slice_center_evolve_array_{}_{}_ky0_green_mul{}.pdf'.format(doping, case, muti_num))
                plt.show()
                ...
                # save_file_name = '{}_mul{}_int{}_start_{}_factor{}_savefile.txt'.format('_'.join(file_name.split('/')[1:]), muti_num, interval_step, start_time, factor)
                ...
                plt.ylim([-0.2,0.2])
                plt.tight_layout()   
                # plt.savefig('./Edge_effect/ladder_trans_2N/{}_mul{}_int{}_start_{}_long_run.pdf'.format('_'.join(file_name.split('/')[1:]), muti_num, interval_step, start_time))

...
for files in ['../ladder_system_test/{}_results'.format(args.doping)]:
    ...
    for case in ['U{}'.format(args.U)]:
        ...
        # projection_points_list = [[0,1],[0,1,2],[-1,0,1],[-1,0,1,2],[-2,-1,0,1,2]]
        # projection_points_list = [[-2,-1,0,1,2],[-2,-1,0,1,2,5,10]]
        # projection_points_list = [[-1,0,1,5,10,15,20]]    # slightly unstable 1E-3, 1E-2 stable
        # projection_points_list = [[-1,0,1]]                 # 1E-4 stable
        # projection_points_list = [[-2,-1,0,1,2]]            # 1E-4 unstable, 1E-3 stable
        projection_points_list = [[0,1,2,4,8,16,32]]            # 1E-4 unstable, 1E-3 stable
        ...
        for m_idx, muti_num in enumerate(muti_num_grid):
            ...
            for i_idx, projection_points in enumerate(projection_points_list):
                ...
                overlap_pinv = np.linalg.pinv(overlap,rcond=1e-3)

                lb_Ci = dict()
                overlap_larger_pinv = np.linalg.pinv(overlap_larger,rcond=1e-3)
